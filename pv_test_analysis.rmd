...
There are a few things to mention from the above PCoAs. The one that stands out the most to me is the Pickle MoBio samples are substantially different than the Pickle Homemade DNA extraction samples. This goes for both the rhizosphere and root. I really do not have any idea what this is about. It doesn't make that much biological sense. The other thing to note is that the the differences between the rhizosphere and root samples appear to be  larger for the MoBio samples than the Homemade DNA extraction samples.
...
```
Here we can see that the soil types and compartments are separating nicely with just the homemade extraction samples. Separation is better in the rhizosphere samples, but this is fairly typical.

```{r}
hmc <- widen(p_data %>% mutate(logRA = log2(RA * 1000 + 1)) %>% filter(Prep == "Homemade" & Site != "x"), value = "logRA", return_df = T)
hmc_m <- grab_metadata(p_data %>% filter(Prep == "Homemade" & Site != "x") %>% select(Compartment, SampleID, Prep, Site), return_df = T)

adonis(hmc ~ Compartment * Site, data = hmc_m)
hmc[1:5,1:5]
